(function (global, factory) {
    ...
    class MapSite extends GW__namespace.map.Map {
        constructor(width, height) {
            super(width, height);
            this.machineCount = 0;
            this.machineId = new GW__namespace.grid.NumGrid(width, height);
        }
        hasItem(x, y) {
        ...
        isBridge(x, y) {
            return this.cellInfo(x, y).hasTileFlag(GW__namespace.tile.flags.Tile.T_BRIDGE);
        }
        isDoor(x, y) {
            return this.cellInfo(x, y).hasTileFlag(GW__namespace.tile.flags.Tile.T_IS_DOOR);
        }
        isSecretDoor(x, y) {
            return this.cellInfo(x, y).hasObjectFlag(GW__namespace.gameObject.flags.GameObject.L_SECRETLY_PASSABLE);
        }
        blocksDiagonal(x, y) {
            return this.cellInfo(x, y).hasObjectFlag(GW__namespace.gameObject.flags.GameObject.L_BLOCKS_DIAGONAL);
        }
        blocksPathing(x, y) {
        ...
        blocksItems(x, y) {
            return this.cellInfo(x, y).hasObjectFlag(GW__namespace.gameObject.flags.GameObject.L_BLOCKS_ITEMS);
        }
        blocksEffects(x, y) {
            return this.cellInfo(x, y).hasObjectFlag(GW__namespace.gameObject.flags.GameObject.L_BLOCKS_EFFECTS);
        }
        isDeep(x, y) {
            return this.cellInfo(x, y).hasTileFlag(GW__namespace.tile.flags.Tile.T_DEEP_WATER);
        }
        isShallow(x, y) {
            if (!this.hasXY(x, y))
                return false;
        ...
        analyze() {
            GW__namespace.map.analyze(this);
        }
        nextMachineId() {
    ...
    class Level {
        constructor(width, height, options = {}) {
            ...
            this.width = width;
            if (options.seed) {
                GW__namespace.random.seed(options.seed);
            }
            this.seq = GW__namespace.random.sequence(width * height);
        ...
        create(setFn) {
            ...
            this.finish(site);
            GW__namespace.utils.forRect(this.width, this.height, (x, y) => {
        ...
        _attachDoor(map, room, x, y, dir) {
            ...
            const isDoor = opts.chance && GW__namespace.random.chance(opts.chance); // did not pass chance
        ...
        cellIsCandidate(builder, blueprint, x, y, distanceBound) {
            ...
            if (this.flags & StepFlags.BF_BUILD_IN_WALLS) {
                ...
                if ((this.item && site.blocksItems(x, y)) ||
                    site
                        .cellInfo(x, y)
                        .hasCellFlag(GW__namespace.map.flags.Cell.IS_CHOKEPOINT |
        ...
        build(builder, blueprint) {
            ...
            do {
                ...
                for (instance = 0; (this.generateEverywhere || instance < instanceCount) &&
                    ...
                    if (this.effect) {
                        DFSucceeded = GW__namespace.effect.fireSync(this.effect, site, x, y);
                    }
    ...
    class Blueprint {
        ...
        pickLocation(site) {
            ...
            if (this.isRoom) {
                ...
                const randSite = GW__namespace.random.matchingLoc(site.width, site.height, (x, y) => site
                    .cellInfo(x, y)
                    .hasCellFlag(GW__namespace.map.flags.Cell.IS_GATE_SITE));
        ...
        computeInterior(builder) {
            ...
            do {
                ...
                if (this.isRoom) {
                    ...
                    let totalFreq = GW__namespace.random.range(this.size[0], this.size[1]); // Keeps track of the goal size.
                        ...
                        for (let n = 0; n < seq.length && qualifyingTileCount < totalFreq; n++) {
                            ...
                            if (distanceMap[i][j] == k) {
                                ...
                                if (site.isOccupied(i, j) ||
                                    site
                                        .cellInfo(i, j)
                                        .hasCellFlag(GW__namespace.map.flags.Cell.IS_IN_MACHINE)) {
        ...
        computeInteriorForVestibuleMachine(builder) {
            ...
            interior.fill(0);
            let qualifyingTileCount = 0; // Keeps track of how many interior cells we've added.
            const totalFreq = GW__namespace.random.range(this.size[0], this.size[1]); // Keeps track of the goal size.
        ...
        clearInteriorFlag(builder) {
            builder.interior.forEach((v, x, y) => {
                ...
                if (!builder.site
                    .cellInfo(x, y)
                    .hasCellFlag(GW__namespace.map.flags.Cell.IS_WIRED |
    ...
    var blueprint = /*#__PURE__*/Object.freeze({
        __proto__: null,
    ...
    class Builder {
        constructor(site, depth) {
            ...
            this.interior = GW__namespace.grid.alloc(site.width, site.height);
            this.occupied = GW__namespace.grid.alloc(site.width, site.height);
            this.viewMap = GW__namespace.grid.alloc(site.width, site.height);
            this.distanceMap = GW__namespace.grid.alloc(site.width, site.height);
        }
        free() {
