...
function choiceRoom(config, grid) {
    ...
    if (Array.isArray(config.choices)) {
        ...
        return utils.ERROR('Expected choices to be either array of choices or map { digger: weight }');
    }
    if (!grid)
        return config;
    let id = choices();
    ...
    if (!digger) {
        return utils.ERROR('Missing digger choice: ' + id);
    }
...
function pickLengthRange(dir, opts) {
    if (dir == utils.UP || dir == utils.DOWN) {
        return range.make(opts.yLength || opts.length || [2, 9]);
    }
    else {
        return range.make(opts.xLength || opts.length || [9, 15]);
    }
}
