import { utils as utils$1, random, path, flag, grid, range, blob, frequency } from 'gw-utils';
...
class GridSite {
    constructor(width, height) {
        ...
        this.flags = grid.alloc(width, height);
        this.choke = grid.alloc(width, height);
    }
    free() {
        grid.free(this.tiles);
        grid.free(this.flags);
        grid.free(this.choke);
    }
    get width() {
...
class BrogueDonut extends RoomDigger {
    ...
    carve(site$1) {
        ...
        if (radius > ringMinWidth + holeMinSize &&
            ...
            utils$1.forCircle(x, y, random.range(holeMinSize, radius - holeMinSize), (x, y) => site$1.setTile(x, y, 0));
        }
        return new Room(x - radius, y - radius, radius * 2 + 1, radius * 2 + 1);
    }
}
...
class Lakes {
    ...
    create(site$1) {
        ...
        const hasWreath = random.chance(this.options.wreathChance)
            ? true
            : false;
...
class Bridges {
    ...
    create(site$1) {
        ...
        const SEQ = random.sequence(site$1.width * site$1.height);
...
class Dungeon {
    ...
    initSeeds() {
        for (let i = 0; i < this.config.levels; ++i) {
            this.seeds[i] = random.number(2 ** 32);
        }
    }
    initStairLocs() {
...
class LoopFinder {
    ...
    _initGrid(site$1) {
        utils$1.forRect(site$1.width, site$1.height, (x, y) => {
    ...
    _update(site$1) {
        ...
        const innerLoop = grid.alloc(site$1.width, site$1.height);
...
export { index$1 as build, index as dig, site };
